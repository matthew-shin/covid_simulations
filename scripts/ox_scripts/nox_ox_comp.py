...
if __name__=="__main__":
    ...
    for i, experiment in enumerate(['3A1','3A2','3A3','3A4']):
        ...
        no_surface, _, _ = get_data(no_datalist[experiment], 48.00)
        frac = no2_surface / (no_surface + no2_surface)
        
        # Control frac
        no2_con, _, _ = get_data(no2_datalist['Con'], 46.00)
        no_con, _, _ = get_data(no_datalist['Con'], 48.00)
        con_frac = no2_con / (no_con + no2_con)
    ...
    cb.set_label('NO2 / NOx (NO + NO2)')
    plt.suptitle('Change in NO2 fraction')
    plt.savefig('surf_no2_noxfrac_diff_map.png')
    plt.show()
