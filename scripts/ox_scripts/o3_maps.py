...
def get_data(dataset, Mr):
    ...
    '''
    surface = dataset[75:135,0,:,:].array.squeeze()*(28.97/Mr)*1e9
...
if __name__=="__main__":
    ...
    for i, experiment in enumerate(['Con', '3A1','3A2','3A3','3A4']):
        ...
        p = axes[i].pcolormesh(lon, lat, surface, cmap='Reds', vmin=0, vmax=100, transform=ccrs.PlateCarree())
    ...
    plt.suptitle('2013 Surface O3 Concentration')
    plt.savefig('surf_o3_map_2013.png')
    '''
    # Set up figure
    ...
    for i, experiment in enumerate(['3A1','3A2','3A3','3A4']):
        ...
        # Draw map data. Give handle p for colorbar use
        p = axes[i].pcolormesh(lon, lat, np.nanmean(diff, axis=0), cmap='RdBu_r', vmin=-5, vmax=5, transform=ccrs.PlateCarree())
        axes[i].set_title(experiment)
    
    cax, kw = matplotlib.colorbar.make_axes(axes, location='bottom', pad=0.1, shrink=0.7)    
    cb = plt.colorbar(p, cax=cax, extend='both', **kw)
    cb.set_label('O3 diff / ppbv')
    plt.suptitle('2014 Mean O3 surface diff')
    plt.savefig('surf_o3_diff_map_2014.png')
