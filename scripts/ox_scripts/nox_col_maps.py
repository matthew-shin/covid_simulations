...
if __name__=="__main__":
    ...
    trop_mask_2014 = load_data('50064', year=2014, experiments=['Con'])
    trop_mask_2013 = load_data('50064', year=2013, experiments=['Con'])
    trop_mask_2012 = load_data('50064', year=2012, experiments=['Con'])
    ...
    for i, experiment in enumerate(['3A1','3A2','3A3','3A4']):
        ...
        # Get data
        surface_2014, lon, lat = get_data(datalist_2014, airmass_2014, trop_mask_2014, experiment, 46.00)
        surface_2013, _, _ = get_data(datalist_2013, airmass_2013, trop_mask_2013, experiment, 46.00)
        surface_2012, _, _ = get_data(datalist_2012, airmass_2012, trop_mask_2012, experiment, 46.00)

        con_2014, _, _ = get_data(datalist_2014, airmass_2014, trop_mask_2014, 'Con', 46.00)
        con_2013, _, _ = get_data(datalist_2013, airmass_2013, trop_mask_2013, 'Con', 46.00)
        con_2012, _, _ = get_data(datalist_2012, airmass_2012, trop_mask_2012, 'Con', 46.00)
        ...
        p = axgr[i].pcolormesh(lon, lat, perc, cmap='RdBu_r', vmin=-75, vmax=75, transform=ccrs.PlateCarree())
    ...
    cb.set_label('Column Change / %')
    plt.suptitle('Percentage Difference in Tropospheric NO2 column')
    plt.savefig('mean_surf_no2_col_perc.png')
    plt.show()
