...
for experiment in experiment_order:
    ...
    o3data = np.nansum(o3data, axis=(1,2,3))
    #print(o3data.shape, area.shape, trop_mask.shape)
    ax.plot(time, o3data/1e9, ls='-', label=experiment, c=colorlist[experiment])
    #ax.set_xticks(np.arange(0,len(time),7))
    #ax.set_xticklabels(time[::7])
ax.legend()
ax.set_ylabel('Tropospheric O3 burden / Tg')
...
for experiment in experiment_order:
    print(experiment)
    ohdata = oh_datalist[experiment].array * airmass[experiment].array
    time = pd.to_datetime([t.strftime() for t in oh_datalist[experiment].coord('time').dtarray])
    ohdata *= trop_mask['Con'].array
    ohdata = np.nansum(ohdata, axis=(1,2,3))

    ax.plot(time, ohdata, ls='-', label=experiment, c=colorlist[experiment])
    #ax.set_xticks(np.arange(0,len(time),7))
    #ax.set_xticklabels(time[::7])
ax.legend()
ax.set_ylabel('Tropospheric OH burden / kg')
ax.set_xlim('2014-01','2015-01')
ax.axvspan('2014-02-16','2014-03-16', alpha=0.1, color='lightgrey')
ax.axvspan('2014-03-16','2014-05-16', alpha=0.2, color='lightgrey')
ax.axvspan('2014-05-16','2014-06-16', alpha=0.1, color='lightgrey')
plt.savefig('trop_oh_burd.png')
plt.show()
'''
