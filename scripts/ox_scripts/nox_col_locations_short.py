...
station_order = ('Beijing','Chengdu','Chonqing','Dalian','Dongguan','Foshan','Guangzhou','Jinan','Nanjing','Qingdao','Shanghai','Shenyang','Tianjin','Wuhan','Xian','Zhengzhou','Milan','Venice','Madrid','Barcelona','Paris','Brussels','Frankfurt','Hamburg','London','Tehran','Isfahan','Daegu','Seoul','New York','Washington D.C.','Philadelphia','Chicago','Detroit')
...
if __name__=="__main__":
    ...
    omi = [station_data[station][1] for station in selected_stations]
    tropomi = [station_data[station][0] for station in selected_stations]
    model = {}
    for experiment in ['3A1','3A2','3A3','3A4']:
        model[experiment] = np.array([model_data[experiment][station] for station in selected_stations]).squeeze().tolist()
        
    tropomi_error = [station_error[station][0] for station in selected_stations]
    omi_error = [station_error[station][1] for station in selected_stations]
    model_error = {}
    for experiment in ['3A1','3A2','3A3','3A4']:
        model_error[experiment] = np.array([model_err[experiment][station] for station in selected_stations]).squeeze().tolist()
    
    x = np.arange(len(selected_stations)/2)
    xbounds = np.arange(-0.5, len(selected_stations)/2 + 0.5, 1)
    width = 0.15
    for i in [0,1]:
        rects1 = axes[i].bar(x - 2.5*width, tropomi[4*i:4*i+4], width, color='lightsteelblue', label='TROPOMI')
        rects2 = axes[i].bar(x - 1.5*width, omi[4*i:4*i+4], width, color='cornflowerblue', label='OMI')
        rects3 = axes[i].bar(x - 0.5*width, model['3A1'][4*i:4*i+4], width, color='mistyrose', label='A1')
        rects4 = axes[i].bar(x + 0.5*width, model['3A2'][4*i:4*i+4], width, color='lightcoral', label='A2')
        rects5 = axes[i].bar(x + 1.5*width, model['3A3'][4*i:4*i+4], width, color='indianred', label='A3')
        rects6 = axes[i].bar(x + 2.5*width, model['3A4'][4*i:4*i+4], width, color='maroon', label='A4')
  
        axes[i].errorbar(x - 2.5*width, tropomi[4*i:4*i+4], yerr=tropomi_error[4*i:4*i+4], capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0.5)
        axes[i].errorbar(x - 1.5*width, omi[4*i:4*i+4], yerr=omi_error[4*i:4*i+4], capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0.5)
        axes[i].errorbar(x - 0.5*width, model['3A1'][4*i:4*i+4], yerr=model_error['3A1'][4*i:4*i+4], capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0.5)
        axes[i].errorbar(x + 0.5*width, model['3A2'][4*i:4*i+4], yerr=model_error['3A2'][4*i:4*i+4], capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0.5)
        axes[i].errorbar(x + 1.5*width, model['3A3'][4*i:4*i+4], yerr=model_error['3A3'][4*i:4*i+4], capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0.5)
        axes[i].errorbar(x + 2.5*width, model['3A4'][4*i:4*i+4], yerr=model_error['3A4'][4*i:4*i+4], capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0.5)

        for j in xbounds:
            axes[i].axvline(j, lw=1, color='gray', ls='--')
        axes[i].axhline(0, lw=1, color='k')
        axes[i].set_xlim(-0.6, len(selected_stations)/2 - 0.4)
        axes[i].set_ylim(-75, 50)
        axes[i].set_xticks([])
        axes[i].set_xticklabels([])
        for k in range(4):
            axes[i].text(0.14+0.24*k, 0.9, selected_stations[4*i:4*i+4][k], transform=axes[i].transAxes, horizontalalignment='center')
    ax1.legend(bbox_to_anchor=(0.5, 1.1), ncol=3, loc='center', frameon=False)
    ax1.text(0.05, 0.5, r'NO$_2$ column change / %', rotation=90, transform=fig.transFigure, va='center')

    plt.savefig('NO2_column_diff_satellite_comparison_short.jpg')
