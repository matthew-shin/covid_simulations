...
def load_F_data(year, version="F", experiments=['Con','3A1','3A2','3A3','3A4']):
    ...
    for experiment in experiments:
        ...
        if version=="F":
            dataset, _ = F(path, 74, 134)
        elif version=="F_clean":
            dataset, _ = F_clean(path, 74, 134)
        elif version=="F_clear_clean":
            dataset, _ = F_clear_clean(path, 74, 134)
        datalist[experiment] = dataset
    return datalist
...
def F_clear_clean(path,start,finish):
    '''
    Total net incoming clear, clean radiation : In - Out
    Returns the total map and area-weighted mean time series
    '''
    sw_in=nc.Dataset(path+'/01207/01207_v4.nc')
    sw_in=sw_in.variables['UM_m01s01i207_vn1105'][start:finish,:,:]
   
    sw_out_clear_clean=nc.Dataset(path+'/01519/01519_v4.nc')
    sw_out_clear_clean=sw_out_clear_clean.variables['UM_m01s01i519_vn1105'][start:finish,85,:,:]
   
    lw_out_clear_clean=nc.Dataset(path+'/02519/02519_v4.nc')
    lw_out_clear_clean=lw_out_clear_clean.variables['UM_m01s02i519_vn1105'][start:finish,85,:,:]
   
    flux = sw_in - sw_out_clear_clean - lw_out_clear_clean
...
if __name__=="__main__":
    ...
    FClearCleanDataset2014 = load_F_data(2014, version="F_clear_clean")
    FClearCleanDataset2013 = load_F_data(2013, version="F_clear_clean")
    FClearCleanDataset2012 = load_F_data(2012, version="F_clear_clean")

    AerDataset2014 = {experiment:FCleanDataset2014[experiment]-FClearCleanDataset2014[experiment] for experiment in FCleanDataset2014}
    AerDataset2013 = {experiment:FCleanDataset2013[experiment]-FClearCleanDataset2013[experiment] for experiment in FCleanDataset2013}
    AerDataset2012 = {experiment:FCleanDataset2012[experiment]-FClearCleanDataset2012[experiment] for experiment in FCleanDataset2012}
    ...
    plt.suptitle('Cloud TOA RF')
    plt.savefig('aer-cloud_95_RF_maps.jpg')
    #plt.show()
