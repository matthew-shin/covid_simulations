...
def get_data(dataset, Mr):
    ...
    '''
    zonal = dataset[75:135,:,:,:]*(28.97/Mr)*1e15
...
if __name__=="__main__":
    ...
    cb.set_label('OH Conc / ppqv')
    for ax in [ax1, ax4, ax5]:
        ax.set_yticks([])
    for ax in [ax2, ax3]:
        ax.set_ylabel('Altitude / m')
    plt.suptitle('2013 OH Zonal Mean Concentration')
    plt.savefig('zon_oh_2013.png')
    ...
    cax, kw = matplotlib.colorbar.make_axes(axes, location='bottom', pad=0.1, shrink=0.7)    
    cb = plt.colorbar(p, cax=cax, extend='both', **kw)
    cb.set_label('OH zonal diff / ppqv')
    for ax in [ax4, ax2]:
        ax.set_yticks([])
    for ax in [ax1, ax3]:
        ax.set_ylabel('Altitude / m')
    plt.suptitle('2013 Difference in zonal mean OH')
    plt.savefig('zon_oh_diff_2013.png')
    ...
    for ax in [ax1, ax3]:
        ax.set_ylabel('Altitude / m')
    plt.suptitle('2013 Percentage difference in zonal mean OH')
    plt.savefig('zon_oh_perc_2013.png')
