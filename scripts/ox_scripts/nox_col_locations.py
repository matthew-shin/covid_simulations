...
if __name__=="__main__":
    ...
    for i, experiment in enumerate(['3A1','3A2','3A3','3A4']):
        ...
        model_error = np.array([model_err[experiment][station] for station in station_order]).squeeze().tolist()
        print(model_error)
        ...
        rects2 = axes[i].bar(x, omi, width, label='OMI')
        rects3 = axes[i].bar(x + width, model, width, label='MODEL')
        axes[i].errorbar(x - width, tropomi, yerr=tropomi_error, capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0)
        axes[i].errorbar(x, omi, yerr=omi_error, capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0)
        axes[i].errorbar(x + width, model, yerr=model_error, capsize=3, fmt='none', ecolor='k', elinewidth=0.5, lw=0)
        for j in xbounds:
            axes[i].axvline(j, lw=1, color='gray', ls='--')
        axes[i].axhline(0, lw=1, color='k')
        axes[i].set_xlim(-1, len(station_order))
    ...
    plt.savefig('NO2_column_diff_satellite_comparison_v2.png')
